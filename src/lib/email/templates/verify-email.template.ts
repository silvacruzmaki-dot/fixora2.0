...
export interface VerifyEmailTemplateResult {
  subject: string;
...
const VERIFY_EMAIL_COPY = {
  es: {
    ...
    preheader:
      "Utiliza este código para verificar tu cuenta de FIXORA.",
    ...
    introduction:
      "Gracias por crear tu cuenta en FIXORA. Para completar el registro, ingresa el siguiente código de verificación:",
    ...
    ignore:
      "Si no creaste una cuenta en FIXORA, puedes ignorar este correo de forma segura.",
  ...
  en: {
    ...
    preheader:
      "Use this code to verify your FIXORA account.",
    ...
    introduction:
      "Thank you for creating your FIXORA account. To complete your registration, enter the following verification code:",
    ...
    ignore:
      "If you did not create a FIXORA account, you can safely ignore this email.",
...
function getTemplateYear(
  value:
    VerifyEmailTemplateInput["generatedAt"],
): number {
  if (
    value === undefined
  ) {
...
function getExpirationMessage(
  minutes: number,
  copy:
    | typeof VERIFY_EMAIL_COPY.es
    | typeof VERIFY_EMAIL_COPY.en,
): string {
  if (minutes === 1) {
    return copy
      .expirationSingular;
...
export function createVerifyEmailTemplate({
  ...
  generatedAt,
}: VerifyEmailTemplateInput): VerifyEmailTemplateResult {
  if (
    !isValidAuthCode(
      code,
    )
  ...
  const escapedExpirationMessage =
    escapeHtml(
      expirationMessage,
    );
